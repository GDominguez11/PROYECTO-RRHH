...
$(".datos-usuario").load("demo.txt", function (responseTxt, statusTxt, xhr)  {
  const tbl = document.querySelector(".datos-usuario");
...
// Agregar la cadena JSON como un valor en FormData
  ...
  $.ajax({
    url: "../controllers/oficialesControlador.php",
    type: "POST",
    data: formData,
    ...
    contentType: false,
      success: function(data) {
        if(Object.entries(data).length === 0){
          ...
          const cellTexto = document.createTextNode(`No hay oficiales`);
        ...
        bitacora.forEach(registro => {
          ...
          const cellID = document.createTextNode(`${registro.id_oficial}`);
          const cellNombre = document.createTextNode(`${registro.nombre_completo}`);
          const cellGrado = document.createTextNode(`${registro.grado}`);
