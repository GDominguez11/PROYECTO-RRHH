...
class usuarioControlador extends usuarioModelo{
	...
	public function agregarEmpleado(){
		...
		$nombres=ConexionBD::limpiar_cadena($_POST['nombre_nuevo']);
		$apellidos=ConexionBD::limpiar_cadena($_POST['apellido_nuevo']);
		...
		if(ConexionBD::verificar_datos("[A-ZÁÉÍÓÚÑa-záéíóú ]{1,50}",$nombres)){
			$alerta=[
				"Alerta"=>"simple",
		...
		if(ConexionBD::verificar_datos("[A-ZÁÉÍÓÚÑa-záéíóú ]{1,50}",$apellidos)){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
		...
		if(ConexionBD::verificar_datos("[a-zñ_0-9]{1,30}",$usuario)){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
		...
		if(ConexionBD::verificar_datos("[A-ZÁÉÍÓÚa-záéíóúÑñ0-9 .,]{1,300}",$direccion)){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
				"Texto"=>"El campo Dirección solo acepta letras, números, espacios y ciertos 
		...
		if(ConexionBD::verificar_datos("[a-z0-9.@]{1,50}",$correo) ){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
				"Texto"=>"El campo Correo electrónico solo acepta letras, números y puntos(.)",
				"Tipo"=>"error"
				];
			echo json_encode($alerta);
			exit();
		...
		if(ConexionBD::verificar_datos("[a-zA-Z0-9$@.%&#_ ]{5,25}",$contrasena) ){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
				"Texto"=>"El campo Contraseña solo acepta letras, números y ciertos 
	...
	public function actualizarEmpleado(){	
		...
		//datos para la tabla de Personas
		$nombres=ConexionBD::limpiar_cadena($_POST['nombre_act']);
		$apellidos=ConexionBD::limpiar_cadena($_POST['apellido_act']);
		...
		if(ConexionBD::verificar_datos("[a-z0-9.@]{1,50}",$correo) ){
			$alerta=[
				"Alerta"=>"simple",
				"Titulo"=>"Ocurrió un error inesperado",
				"Texto"=>"El campo Correo electrónico solo acepta letras, números y puntos(.), sin espacios",
				"Tipo"=>"error"
				];
			echo json_encode($alerta);
			exit();
		...
		if(ConexionBD::verificar_datos("[A-ZÁÉÍÓÚa-záéíóúÑñ0-9 .,]{1,300}",$direccion)){
			...
			if(($actualizar_persona->rowCount()==1) && ($actualizar_usuario->rowCount()==1)){
				...
				$datos_bitacora = [
					...
					"descripcion" => "El usuario ".$_SESSION['usuario_login']." actualizó la información de un usuario"
				];
				$resultado=$registroEvento->guardar_bitacora($datos_bitacora);
	...
	public function actualizarPerfil(){	
		...
		//datos para la tabla de Personas
		$nombres=ConexionBD::limpiar_cadena($_POST['nombre_perfil_act']);
		$apellidos=ConexionBD::limpiar_cadena($_POST['apellido_perfil_act']);
		...
		$actualizarContrasena=usuarioModelo::actualizarContrasenaUsuario($datos_clave_act,$id_actualizacion);
			...
			if(($actualizarContrasena->rowCount()==1)){
				...
				$datos_bitacora = [
					...
					"descripcion" => "El usuario ".$_SESSION['usuario_login']." cambió su contraseña"
				];
				$resultado=$registroEvento->guardar_bitacora($datos_bitacora);
	...
	public function autoregistroUsuario(){
		...
		//datos para la tabla de Personas
		$nombres=ConexionBD::limpiar_cadena($_POST['nombre_autoregistro']);
		$apellidos=ConexionBD::limpiar_cadena($_POST['apellido_autoregistro']);
		...
		if($revisarUsuario->rowCount()>0){
			...
			if(($agregar_usuario->rowCount()==1) && ($agregar_persona->rowCount()==1) ){
				...
				$datos_bitacora = [
					...
					"id_usuario" => 1,
					"accion" => "Inserción",
					"descripcion" => "Autoregistro de usuario"
				];
				$resultado=$registroEvento->guardar_bitacora($datos_bitacora);
